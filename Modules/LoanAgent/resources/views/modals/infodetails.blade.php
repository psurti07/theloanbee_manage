...
<div class="modal fade" id="convertCustomerModal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="exampleModalLabel1" aria-hidden="true" style="backdrop-filter: brightness(0.7) blur(2px);">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form method="post" action="{{ route('manage.loanagent.companyleads.convertcustomer') }}" class="convert-customer-form" id="convert-customer-form">
        ...
        $("#block-customer").on('click', function() {
            swal({
                ...
                if (willBlock) {
                    ...
                    $.ajax({
                        url: '{!! route("manage.loanagent.companyleads.block.user")  !!}',
                        type: 'POST',
                        data: JSON.stringify({
                            id: userId
                        }),
        ...
        $("#stop-remarketing").on('click', function() {
            swal({
                ...
                if (willDND) {
                    let userId = $(this).data('userid');
                    $.ajax({
                        url: '{!! route("manage.loanagent.companyleads.dnd.user")  !!}',
                        type: 'POST',
                        data: JSON.stringify({
                            id: userId
                        }),
        ...
        $("#delete-customer").on('click', function() {
            swal({
                ...
                if (willDelete) {
                    let userId = $(this).data('userid');
                    $.ajax({
                        url: '{!! route("manage.loanagent.companyleads.destroy.user")  !!}',
                        type: 'POST',
                        data: JSON.stringify({
                            id: userId
                        }),
