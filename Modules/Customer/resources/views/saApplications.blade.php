...
<div class="container-fluid">
    <div class="row g-3">
        ...
        <div class="col-md-2 position-relative">
            ...
            <select class="form-select" name="acStatus" id="acStatus">
...
<script>
    $(document).on("init.dt",'#saapplications-table', function() { // Ensures full page load
        let today = new Date();
        ...
        //let acStatus = sessionStorage.getItem('acStatus') || new URLSearchParams(window.location.search).get('acStatus')
        let agent = sessionStorage.getItem('agents') || new URLSearchParams(window.location.search).get('agents')

        // Set date input fields
        $('#fromdate').val(fromDate);
        $('#todate').val(toDate);
        //$('#acStatus').val(acStatus);
        $('#agents').val(agent);

        let table = $("#saapplications-table").DataTable(); // Get existing DataTable instance

        table.on('preXhr.dt', function(e, settings, data) {
            data.start_date = $("#fromdate").val();
            data.end_date = $("#todate").val();
            //data.acStatus = $("#acStatus").val();
        ...
        //sessionStorage.removeItem('acStatus');
